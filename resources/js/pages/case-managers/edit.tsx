...
interface CaseManagersEditProps {
...
export default function CaseManagersEdit({ caseManager, agencies }: CaseManagersEditProps) {
    const { data, setData, put, processing, errors } = useForm({
        name: caseManager.name,
        email: caseManager.email,
        phone: caseManager.phone || '',
        agency_id: caseManager.agency_id,
    });

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/case-managers/${caseManager.id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit ${caseManager.name}`} />
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-6">
                <div className="flex items-center justify-between">
                    <Heading title={`Edit ${caseManager.name}`} />
                    <Link
                        href="/case-managers"
                ...
                <div className="border-sidebar-border/70 dark:border-sidebar-border rounded-xl border bg-card p-6">
                    <form onSubmit={submit} className="space-y-6">
                        ...
                        <div className="flex items-center justify-end space-x-4">
                            ...
                            >
                                {processing ? 'Updating...' : 'Update Case Manager'}
                            </button>
                        </div>
                    </form>
                </div>
